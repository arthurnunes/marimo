...
def init():
    ...
    return alt, csv, mo, os, path, pd, pl, table_name
...
@app.cell(hide_code=True)
def __(bar_season_chart, line_season_chart, mo):
...
@app.cell
def __(mo, selected_constructor_id, selected_driver_id, selected_year):
...
@app.cell
def __(mo, pl, race_drilldown_chart):
...
@app.cell
def __(mo, selected_driver_id, selected_race_id):
    race_stats = mo.sql(
        f"""
        WITH LapAnalysis AS (
            SELECT 
